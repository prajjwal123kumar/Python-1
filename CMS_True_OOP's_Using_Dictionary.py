...
class Customer:
    dict_cus={} # Static Member it has only one memory blocks.
    def __init__(self):
        self.id=0
        self.name=""
        self.address=""
    ...
    def add_customer(self): # self=get data to add
        Customer.dict_cus[self.id]=self
    def search_customer(self,cus_id):
        e=Customer.dict_cus.get(cus_id,'NA')
        if e!='NA':
            self.id=e.id
            self.name=e.name
...
def show_all_customer():
        for cus in Customer.dict_cus.values():
            print(cus)
while(True):
